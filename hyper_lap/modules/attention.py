...
class Attention(eqx.Module):
    ...
    def __call__(self, x: Float[Array, "n d"]) -> Float[Array, "n d"]:
        ...
        k = self.transpose_for_scores(jax.vmap(self.key)(x))
        v = self.transpose_for_scores(jax.vmap(self.value)(x))

        assert_shape([q, k, v], [self.num_heads, n_seq, self.dim_head])
        ...
        out = out.swapaxes(0, 1).reshape(n_seq, self.num_heads * self.dim_head)
...
class SpatialAttention(eqx.Module):
    ...
    def __call__(self, x: Float[Array, "c h w"]) -> Float[Array, "c h w"]:
        ...
        qkv = self.qkv(x).reshape(3, self.num_heads, self.dim_head, h * w).swapaxes(2, 3)

        assert_shape(qkv, [3, self.num_heads, h * w, self.dim_head])
        ...
        assert_shape([q, k, v], [self.num_heads, h * w, self.dim_head])

        out = jax.nn.dot_product_attention(q, k, v)

        assert_shape(out, [self.num_heads, h * w, self.dim_head])
